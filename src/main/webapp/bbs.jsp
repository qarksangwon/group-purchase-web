...
<body>
	<%
		...
		if(session.getAttribute("userID")!= null){
			...
			String userUC = new UserDAO().getUser(userID).getUserUC();
		}
		int pageNumber =1;
		...
		<div class = "row">
			...
			<thead>
				<tr>
					...
					<th style="background-color: #eeeeee; text-align : center;">모집시간</th>
				</tr>
			</thead>
			<tbody>
				<%
					...
					<td><%=list.get(i).getUserUC() %></td>
					<td><%=list.get(i).getBbsDate().substring(0,11) + list.get(i).getBbsDate().substring(11,13)+"시"+list.get(i).getBbsDate().substring(14,16)+"분" %></td>
					<td><%=list.get(i).getBbsTime() %></td>
					</tr>
						<%
					}
				%>
			...
			<%
				if(pageNumber != 1){
			%>		
				<a href="bbs.jsp?pageNumber=<%=pageNumber-1%>"class="btn btn-info btn-arraw-left">이전</a>
			<%
				}if(bbsDAO.nextPage(pageNumber +1)){
					%>
				<a href="bbs.jsp?pageNumber=<%=pageNumber+1%>"class="btn btn-info btn-arraw-left">다음</a>
					<%
				}
			%>
			<a>주문하고자 하는 메뉴를 누르면 해당 게시판으로 이동합니다.</a>
